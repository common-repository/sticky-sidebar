...
if(isset($_REQUEST['sidebardel']) && $_REQUEST['sidebardel']!=''){
	...
	print_r($stky_option);
	if($key!==false){
		if(delete_option( 'ri_sticky_widget_'.$_REQUEST['sidebardel'] )){
			unset($stky_option[$key]);
		}else{ unset($stky_option[$key]); }
		if(update_option( 'ri_sticky_widget_id', $stky_option )){  }
	}
...
if(isset($_POST['ssn'])){
	...
	if(!get_option( 'ri_sticky_widget_'.$sw_data )){ 
		if(add_option( 'ri_sticky_widget_'.$sw_data , $ssbdet )){  } 
	}else{  
		
	}
	update_option( 'ri_sticky_widget_id', $stky_option );
}else{  }
...
else{ if(add_option( 'ri_sticky_widget_id' )){  } }
...
?>
    ...
    <thead>
    	<tr><td class="column-date"> S. No. </td> <td> Sticky sidebar name </td> <td> Description </td> <td>Shortcode</td><td>Remove</td></tr>
    </thead>
    <?php if($stky_option): $i=1;
		...
		if(get_option( 'ri_sticky_widget_'.$val )){ $snmd = get_option( 'ri_sticky_widget_'.$val ); }
            ...
            <tr> 
                ...
                <td><label><?php echo '[sticky_sidebar '.$val.']'; ?></label></td>
                <td><a href="admin.php?page=Sticky-Widget/main.php&sidebardel=<?php echo $val; ?>">Remove</a></td>
            </tr>
            <?php $i++;
		endforeach;
...
<form method="post">
	<p><input type="text" placeholder="Sticky Sidebar Name" name="ssn" />
